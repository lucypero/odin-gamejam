...
CreateLight :: proc(type: LightType, position, target: [3]f32, attenuation: f32, color: rl.Color, shader: rl.Shader) -> (light: Light) {
	if lightsCount < MAX_LIGHTS {
		...
		light.attenuationLoc = i32(rl.GetShaderLocation(shader, rl.TextFormat("lights[%i].attenuation", lightsCount)))
...
UpdateLightValues :: proc(shader: rl.Shader, light: Light) {
	...
	rl.SetShaderValue(shader, rl.ShaderLocationIndex(light.attenuationLoc), &light.attenuation, .FLOAT)
